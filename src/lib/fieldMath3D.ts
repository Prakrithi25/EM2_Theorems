// fieldMath3D.ts
// 3D vector field definitions plus the numerical machinery Stokes' and Gauss's
// modules use: surface flux of curl, line integrals around a rim, closed-surface
// flux, and volume integrals of divergence.
...
export interface Field3D {
  ...
  description: string;
  F: (p: Vec3) => Vec3;
  /** analytic curl ∇×F, constant or position-dependent */
  curl: (p: Vec3) => Vec3;
}

// Fields chosen so curl is easy to reason about and non-trivial to look at.
...
/** Numeric curl via central finite differences, used to cross-check the analytic curl. */
...
/**
 * Parametrized dome surface over a disk of given base radius, height controls how "puffed up" it is.
 * height = 0 -> flat disk (still a valid surface with the same rim).
 * u,v in [0,1]x[0,1]: u = radial fraction, v = angular fraction.
 */
export function domePoint(u: number, v: number, baseRadius: number, height: number): Vec3 {
  const r = u * baseRadius;
  const theta = v * Math.PI * 2;
  // height profile: paraboloid-like bump that is 0 at the rim (r = baseRadius) and `height` at center
  const z = height * (1 - (r * r) / (baseRadius * baseRadius));
  return { x: r * Math.cos(theta), y: r * Math.sin(theta), z };
}

/** Rim (boundary curve) of the dome — always the same circle regardless of height. */
...
/** Tangent direction along the rim at parameter t in [0,1] (counter-clockwise). */
export function domeRimTangent(_baseRadius: number, t: number): Vec3 {
  const theta = t * Math.PI * 2;
  return { x: -Math.sin(theta), y: Math.cos(theta), z: 0 };
}

/** ∮ F·dr around the circular rim via the midpoint rule. */
...
/** Partial rim circulation up to parameter t in [0,1], for the animated running total. */
...
/**
 * ∬_S (curl F)·dS over the dome surface via a grid of quads in (u,v) parameter space.
 * Uses the cross product of partial derivatives (numerically) as the area-weighted normal.
 */
export function surfaceCurlFlux(
  ...
  for (let i = 0; i < nu; i++) {
    for (let j = 0; j < nv; j++) {
      ...
      // normal (area-weighted, outward/upward) = ∂p/∂u × ∂p/∂v scaled by du*dv
      let normal = v3.cross(pu, pv);
      // ensure upward-ish orientation (consistent with CCW rim = outward normal has positive z bias)
...
export interface DivergenceField {
  strength: number; // negative = sink, positive = source
  F: (p: Vec3) => Vec3;
  divergence: (p: Vec3) => number;
}

/** Radial field F = k·(x,y,z) has constant divergence 3k — sign of k sets source/sink. */
...
/** ∬ F·dS over a sphere of given radius, via a lat/long grid. */
...
/** ∭ div F dV over a solid sphere of given radius (analytic-friendly numeric integration). */
export function sphereVolumeIntegral(field: DivergenceField, radius: number, nR = 30): number {
  // divergence is constant for our radial field, so this reduces to div * volume,
  // but we integrate numerically (radial shells) to keep the pipeline generic/testable.
  let total = 0;
  const dr = radius / nR;
  for (let i = 0; i < nR; i++) {
    const r = (i + 0.5) * dr;

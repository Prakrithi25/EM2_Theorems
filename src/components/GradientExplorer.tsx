...
export default function GradientExplorer({ onReadouts }: Props) {
  ...
  function draw() {
    ...
    drawArrow(ctx, p0, worldToScreen({ x: point.x + gUnit.x * 0.9, y: point.y + gUnit.y * 0.9 }), 1.6, cssVars.teal);

    // direction vector (draggable)
    const dir = { x: Math.cos(angle), y: Math.sin(angle) };
    drawArrow(ctx, p0, worldToScreen({ x: point.x + dir.x * 0.9, y: point.y + dir.y * 0.9 }), 1.6, cssVars.amber);
  ...
  function handlePointerDown(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (distToHandle < 0.4) dragRef.current = 'dir';
    else if (distToPoint < 0.5) dragRef.current = 'point';

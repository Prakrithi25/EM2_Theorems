...
function Particles({ strength, radius, running }: { strength: number; radius: number; running: boolean }) {
  ...
  // Each particle keeps a persistent radial "lane" (direction + phase) so it looks like
  // continuous flow rather than jitter, and respawns near the center after reaching the edge
  // (source) or near the edge after reaching the center (sink).
  const lanes = useMemo(() => {

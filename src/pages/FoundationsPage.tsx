...
export default function FoundationsPage() {
  ...
  const aligned = gradMag > 1e-6 ? dirDeriv / gradMag : 0; // cos(angle between direction and gradient)
      ...
      guide={
        <>
          ...
          <Section title="Differentiation of a vector · velocity & acceleration">
            <p>
              For a position vector <Inline tex="\mathbf{r}(t) = x(t)\hat{\imath} + y(t)\hat{\jmath} + z(t)\hat{k}" /> tracing
              a particle's path, the derivative with respect to time gives velocity, and the second
          ...
          <Section title="Scalar potential of a vector function">
            <p>
              ...
              scalar function, <Inline tex="\mathbf{F} = \nabla \phi" />, then <Inline tex="\phi" /> is
              called its <em>scalar potential</em>. Not every field has one — only{' '}
          ...
          <Section title="Flow of a compressible fluid · incompressibility">
            ...
            <p>
              ...
              density along each fluid particle doesn't change, which reduces this to the condition{' '}
              <Inline tex="\nabla\cdot\mathbf{v} = 0" /> — exactly the solenoidal condition above.
              This is what the Gauss's theorem module's "sink/source" slider is deliberately
              violating when set away from zero.
            </p>
          </Section>

          <Section title="Conservative forces">
            <p>
              A force field <Inline tex="\mathbf{F}" /> is <strong style={{ color: 'var(--ink)' }}>conservative</strong> if
              it's irrotational, so <Inline tex="\mathbf{F} = -\nabla V" /> for some potential energy
              function <Inline tex="V" />. Work done moving between two points then depends only on
              the endpoints, not the path — which is exactly what "area of a plane region as a line
          ...
          <Section title="Unit IV — the general integral concepts behind each module">
            <p>
              ...
              <Inline tex="\int_C \mathbf{F}\cdot d\mathbf{r}" /> — accumulate a field along a curve.
              This is the left-hand side of both Green's and Stokes' theorems.
            </p>
            <p>
              <strong style={{ color: 'var(--ink)' }}>Surface integral:</strong>{' '}
              <Inline tex="\iint_S \mathbf{F}\cdot d\mathbf{S}" /> — accumulate a field's flux through
              a surface. Right-hand side of Stokes', left-hand side of Gauss's.
            </p>
            <p>
              <strong style={{ color: 'var(--ink)' }}>Volume integral:</strong>{' '}
              <Inline tex="\iiint_V g \, dV" /> — accumulate a scalar over a 3D region. Right-hand
              side of Gauss's theorem.
            </p>
            <p>
              <strong style={{ color: 'var(--ink)' }}>Work done by a force:</strong> exactly a line
              integral, <Inline tex="W = \int_C \mathbf{F}\cdot d\mathbf{r}" />. For a conservative
              force this equals <Inline tex="V(\text{start}) - V(\text{end})" /> — see the Gauss and
              Green modules' circulation readouts.
            </p>
            <p>
              <strong style={{ color: 'var(--ink)' }}>Determining a potential function:</strong> given
              an irrotational <Inline tex="\mathbf{F}" />, integrate{' '}
              <Inline tex="\partial \phi/\partial x = F_1" /> etc. component-by-component (up to a
              constant) to reconstruct <Inline tex="\phi" />.
            </p>
            <p>
              <strong style={{ color: 'var(--ink)' }}>Area as a boundary line integral:</strong> a
              direct corollary of Green's theorem — pick <Inline tex="P = -y/2, Q = x/2" /> and the
              area formula drops out:
            </p>
